...
contract LotteryProject {
    // State Variables
    address payable  public adminManager; // manager ที่อยู่ของผู้จัดการ (manager) ของลอตเตอรี
    ...
    function selectWinners() public onlyAdmin {
        //require(msg.sender == adminManager, "Only the manager can call this function.");
        require(getLength() >= 5, "At least 5 players required.");

        uint prizePool = getBalance(); // ยอดรวมเงินรางวัล

        // คำนวณจำนวนเงินแต่ละรางวัล 1 2 3
        ...
        payable(adminManager).transfer(managerFee);
